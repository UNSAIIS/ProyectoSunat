...
import funciones.GeneralFunctions;
import funciones.HeaderHandlerResolver;
import funciones.LecturaXML;
...
public class BolElectronica {
    ...
    public static String generarXMLZipiadoBoleta(Cabecera cabecera, List<Detalle> detalle, List<Leyenda> leyendas) {
        org.apache.xml.security.Init.init();
        String resultado = "";

        String caminoo; // = Util.getPathZipFilesEnvio();
        String archiv;
        try {

            caminoo = "F:\\FacturadorSUNAT\\sunat_archivos\\sfs\\ENVIO\\";

            if (cabecera != null) {
                archiv = caminoo + cabecera.getEmpr_nroruc() + "-03-" + cabecera.getDocu_numero() + ".xml";

                resultado = creaXml(cabecera, detalle, leyendas, caminoo);

                resultado = enviarZipASunat(caminoo, cabecera.getEmpr_nroruc() + "-03-" + cabecera.getDocu_numero() + ".zip", cabecera.getEmpr_nroruc());
    ...
    public static String enviarZipASunat(String camino, String archivo, String RUC) {
        String resultado = "";
        try {

            javax.activation.FileDataSource fileDataSource = new javax.activation.FileDataSource(camino + archivo);
            ...
            respuestaSunat = port1.sendBill(archivo, dataHandler);
//            

            String pathRecepcion = "F:\\FacturadorSUNAT\\sunat_archivos\\sfs\\RPTA\\";
            FileOutputStream fos = new FileOutputStream(pathRecepcion + "R-" + archivo);
            fos.write(respuestaSunat);
            fos.close();

            ZipFile archive = new ZipFile(pathRecepcion + "R-" + archivo);
            Enumeration e = archive.entries();
            System.out.println("controlador.BolElectronica.enviarZipASunat()"+e);
            ...
            archivo = archivo.substring(0, archivo.indexOf(".zip"));
            resultado = LecturaXML.getRespuestaSunat(pathRecepcion + "R-" + archivo + ".xml");
            System.out.println("ENVIO ARCHIVO A SUNAT REVISE: "+pathRecepcion);
    ...
    private static String creaXml(Cabecera articulos, List<Detalle> detdocelec, List<Leyenda> leyendas, String ruta) {
        String resultado = "";
        try {
            ...
            String keystoreFile = "F:\\FacturadorSUNAT\\sunat_archivos\\sfs\\CERT\\MiAlmacen.jks";
            ...
            CDATASection cdata;
            String pathXMLFile = ruta + articulos.getEmpr_nroruc() + "-03-" + articulos.getDocu_numero() + ".xml";
            File nomArchivp = new File(pathXMLFile);
            ...
            //agrupa1
            if (!articulos.getDocu_gravada().trim().equals("0.00")) {
                ...
                PayableAmount1.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                PayableAmount1.setIdAttributeNS(null, "currencyID", true);
                envelope.appendChild(PayableAmount1);
                PayableAmount1.appendChild(doc.createTextNode(articulos.getDocu_gravada().trim()));
            ...
            //agrupa2
            if (!articulos.getDocu_inafecta().trim().equals("0.00")) {
                ...
                PayableAmount2.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                PayableAmount2.setIdAttributeNS(null, "currencyID", true);
                envelope.appendChild(PayableAmount2);
                PayableAmount2.appendChild(doc.createTextNode(articulos.getDocu_inafecta().trim()));
            ...
            //agrupa3
            if (!articulos.getDocu_exonerada().trim().equals("0.00")) {
                ...
                PayableAmount3.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                PayableAmount3.setIdAttributeNS(null, "currencyID", true);
                envelope.appendChild(PayableAmount3);
                PayableAmount3.appendChild(doc.createTextNode(articulos.getDocu_exonerada().trim()));
            ...
            //agrupa4
            if (!articulos.getDocu_gratuita().trim().equals("0.00")) {
                ...
                PayableAmount4.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                PayableAmount4.setIdAttributeNS(null, "currencyID", true);
                envelope.appendChild(PayableAmount4);
                PayableAmount4.appendChild(doc.createTextNode(articulos.getDocu_gratuita().trim()));
            ...
            //agrupa5
            if (!articulos.getDocu_descuento().trim().equals("0.00")) {
                ...
                PayableAmount5.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                PayableAmount5.setIdAttributeNS(null, "currencyID", true);
                envelope.appendChild(PayableAmount5);
                PayableAmount5.appendChild(doc.createTextNode(articulos.getDocu_descuento().trim()));
            ...
            String BaseURI = nomArchivp.toURI().toURL().toString();
...
//bloque1
            ...
            ID5.appendChild(doc.createTextNode(articulos.getDocu_numero().trim()));

            Element IssueDate = doc.createElementNS("", "cbc:IssueDate");
            envelope.appendChild(IssueDate);
            IssueDate.appendChild(doc.createTextNode(articulos.getDocu_fecha().trim()));

            Element InvoiceTypeCode = doc.createElementNS("", "cbc:InvoiceTypeCode");
            envelope.appendChild(InvoiceTypeCode);
            InvoiceTypeCode.appendChild(doc.createTextNode(articulos.getDocu_tipodocumento().trim()));

            Element DocumentCurrencyCode = doc.createElementNS("", "cbc:DocumentCurrencyCode");
            envelope.appendChild(DocumentCurrencyCode);
            DocumentCurrencyCode.appendChild(doc.createTextNode(articulos.getDocu_moneda().trim()));
...
//bloque2 cac:Signature--------------------------------------------------------
            ...
            ID6.appendChild(doc.createTextNode(articulos.getEmpr_nroruc().trim()));
            ...
            ID7.appendChild(doc.createTextNode(articulos.getEmpr_nroruc().trim()));
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_razonsocial().trim());
            ...
            URI.appendChild(doc.createTextNode(articulos.getEmpr_nroruc().trim()));
//bloque3 cac:AccountingSupplierParty-----------------------------------------
            ...
            CustomerAssignedAccountID.appendChild(doc.createTextNode(articulos.getEmpr_nroruc().trim()));
            ...
            AdditionalAccountID.appendChild(doc.createTextNode(articulos.getEmpr_tipodoc().trim()));
//***********************************************************
            ...
            ID8.appendChild(doc.createTextNode(articulos.getEmpr_ubigeo().trim()));
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_direccion().trim());
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_provincia().trim());
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_departamento().trim());
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_distrito().trim());
            ...
            cdata = doc.createCDATASection(articulos.getEmpr_pais().trim());
...
// bloque4
            ...
            CustomerAssignedAccountID1.appendChild(doc.createTextNode(articulos.getClie_numero().trim()));
            ...
            AdditionalAccountID1.appendChild(doc.createTextNode(articulos.getClie_tipodoc().trim()));
            ...
            cdata = doc.createCDATASection(articulos.getClie_nombre().trim());
...
//bloque 5
            ...
            TaxAmount.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
            TaxAmount.setIdAttributeNS(null, "currencyID", true);
            TaxTotal.appendChild(TaxAmount);//se anade al grupo TaxTotal
            TaxAmount.appendChild(doc.createTextNode(articulos.getDocu_igv().trim()));
            ...
            TaxAmount1.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
            TaxAmount1.setIdAttributeNS(null, "currencyID", true);
            TaxSubtotal.appendChild(TaxAmount1);//se anade al grupo TaxSubtotal
            TaxAmount1.appendChild(doc.createTextNode(articulos.getDocu_igv().trim()));
...
//bloque 6     
            ...
            if (!articulos.getDocu_descuento().equals("0.00")) {
                Element AllowanceTotalAmount = doc.createElementNS("", "cbc:AllowanceTotalAmount");
                AllowanceTotalAmount.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
                ...
                AllowanceTotalAmount.appendChild(doc.createTextNode(articulos.getDocu_descuento().trim()));
            }

            Element PayableAmount = doc.createElementNS("", "cbc:PayableAmount");
            PayableAmount.setAttributeNS(null, "currencyID", articulos.getDocu_moneda().trim());
            ...
            PayableAmount.appendChild(doc.createTextNode(articulos.getDocu_total().trim()));
//detalle factura
            for (Detalle listaDet : detdocelec) {
...
//
            ...
            sig.setId(articulos.getEmpr_nroruc());
            sig.addKeyInfo(cert);
            {
            ...
            FileOutputStream f = new FileOutputStream(nomArchivp);
            ...
            resultado = GeneralFunctions.crearZip(articulos, ruta, nomArchivp);
